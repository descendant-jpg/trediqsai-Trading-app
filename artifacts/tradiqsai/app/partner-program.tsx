...
import { getAffiliateStats, type AffiliateStats } from '@/services/supabaseService';
...
export default function PartnerProgramScreen() {
  ...
  React.useEffect(() => {
    ...
    getAffiliateStats(session.user.id)
      .then((stats) => { if (!cancelled) setAffiliate(stats); })
      .catch(() => { /* New partners do not have a row yet. */ });
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 105 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.arsenal}><Feather name="edit-3" size={20} color={c.primary} /><View style={{ flex: 1 }}><Text style={styles.arsenalTitle}>Done-For-You Content</Text><Text style={styles.arsenalText}>Get instant access to high-converting TikTok hooks, Instagram Stories, and X templates the moment you are approved.</Text>{affiliate && <Text style={styles.partnerStatus}>{affiliate.tier_level} partner · ${affiliate.total_earnings.toFixed(2)} earned</Text>}</View></View>
...
const styles = StyleSheet.create({
  ...
  arsenal: { flexDirection: 'row', gap: 12, backgroundColor: c.background, borderColor: c.border, borderWidth: 1, borderRadius: 10, padding: 15, marginBottom: 25 }, arsenalTitle: { color: c.foreground, fontSize: 14, fontFamily: 'Inter_700Bold' }, arsenalText: { color: c.mutedForeground, fontSize: 11, lineHeight: 17, marginTop: 5 }, partnerStatus: { color: c.success, fontSize: 10, fontFamily: 'Inter_600SemiBold', marginTop: 8 },
