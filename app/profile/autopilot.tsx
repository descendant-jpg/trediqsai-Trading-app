...
const CYAN = '#00F0FF';
type Bot = { name: string; winRate: string; drawdown: string; description: string };
const bots: Bot[] = [
  { name: 'Pulse Scalper', winRate: '72%', drawdown: '4.8%', description: 'High-frequency momentum entries for liquid session windows.' },
  { name: 'Swing Master', winRate: '68%', drawdown: '7.2%', description: 'Multi-session structure trades with adaptive targets.' },
  { name: 'News Sniper', winRate: '64%', drawdown: '9.1%', description: 'Volatility-aware event reactions with defined risk.' },
];
...
export default function AutoPilotScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const { data } = await supabase.from('profiles').select('active_bot, allocated_capital').eq('id', session.user.id).maybeSingle();
