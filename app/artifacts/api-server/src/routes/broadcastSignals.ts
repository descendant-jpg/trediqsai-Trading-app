...
async function admin(userId: string) {
  const r = await fetch(`${url}/rest/v1/profiles?id=eq.${encodeURIComponent(userId)}&select=role`, { headers: headers() });
  return r.ok && (await r.json() as Array<{ role?: string }>)[0]?.role?.toLowerCase() === "admin";
}
...
router.post("/signals/broadcast", identity(), async (req, res) => {
  ...
  if (!url || !key) return res.status(503).json({ error: "Signal feed is not configured." });
  if (!(await admin(userId))) return res.status(403).json({ error: "Administrator access required." });
