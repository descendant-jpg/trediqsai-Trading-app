...
type Props = { visible: boolean; onClose: () => void; onStatusChange: (enabled: boolean) => void };
type Factor = { id: string; status?: string; factor_type?: string };

export function TwoFactorAuthModal({ visible, onClose, onStatusChange }: Props) {
  ...
  const verify = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setRecoveryCodes((data as any)?.recovery_codes ?? []);
      await refresh();
      message('Two-factor authentication enabled', 'Your authenticator is now protecting this account.');
