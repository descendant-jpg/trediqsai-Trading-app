...
/**
 * The CMS must not initiate Supabase refresh/sign-out side effects. The shared
 * client attaches the current cached bearer token; a rejected token is rendered
 * inline as an exact 401 instead of mutating the mounted auth tree.
 */
export default function AdminDashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    // CMS auth failures must render inline. Do not let the shared 401
    // recovery path sign the user out or replace the mounted admin route.
    return customFetch<AdminDashboardResponse>("/api/admin/dashboard", {
    ...
    <View style={[styles.safeArea, { paddingTop: insets.top }]}>
      <View style={styles.screen}>
        ...
        <View style={styles.statGrid}>
          {statCards.map((stat) => (
            <View key={stat.label} style={styles.statCard}>
              ...
              {loading && !metrics ? (
                <ActivityIndicator size="small" color={CYAN} style={styles.statLoader} />
              ) : (
                <Text style={styles.statValue}>
                  {metrics ? stat.value.toLocaleString() : "—"}
                </Text>
              )}
            </View>
          ))}
        ...
        <View style={styles.recentCard}>
          {!loading && recentPosts.length === 0 && !errorMessage ? (
            <Text style={styles.emptyText}>No market insights have been published yet.</Text>
          ) : null}
