...
- The Git Data API blob endpoint hits secondary rate limits (429) fast: concurrency 20 with no delay fails almost immediately. Use chunks of ~6, honor the `Retry-After` header with backoff, and make the upload loop resumable (blobs are content-addressed and idempotent — re-POSTing returns the same sha). Generate manifests with `git diff --raw --no-abbrev` (abbreviated SHAs are rejected in tree entries), and skip re-upload for R100 renames.
- Deletion (`D`) manifest lines zero out the NEW mode/sha columns — using them verbatim gets a 422 "Must supply a valid tree.mode". For deletions, emit the tree entry with the OLD mode (first column) and `sha: null`.
- After PATCHing the ref, the very first GET can return a stale SHA — sleep ~1.5s and re-read before assuming a repair is needed.
- Checkpoint auto-commits can sweep working-tree changes mid-session, so `git diff HEAD~1..HEAD` may silently omit files you edited. Build push manifests from the explicit list of paths you changed, uploading full blob contents, not from a single-commit diff.