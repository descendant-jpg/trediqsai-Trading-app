...
Former repeat offender, now removed: a stale `routing-release/` copy of the app had its own auto-starting workflows that booted duplicate servers on the same ports (18130 Expo, 8080 API) and won the port race after restarts; the real Expo workflow then froze on an interactive "Use port N instead? (Y/n)" prompt (white screen) and the API died with EADDRINUSE. Deleting just its `artifact.toml` manifests did NOT stop the platform from reviving those workflows — the whole `routing-release/` directory was deleted (2026-08-24; recoverable from git history). If port hijacking ever recurs, check for another stray full-tree copy of the app, not just stray processes.
