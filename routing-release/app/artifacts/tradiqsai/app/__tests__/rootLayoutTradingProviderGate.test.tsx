...
/**
 * Integration test: verifies that TradingProvider is mounted inside
 * RootLayout using a dedicated single-component spy — the same vi.fn pattern
 * used for SubscriptionProvider in rootLayoutSubscriptionProviderGate.test.tsx,
 * BiometricLock in rootLayoutBiometricLockGate.test.tsx, MfaGate in
 * rootLayoutMfaGate.test.tsx, and DegradedSecurityNoticeProvider in
 * rootLayoutDegradedSecurityGate.test.tsx.
 *
 * TradingProvider wraps the core trading context used throughout the app.
 * Removing it silently would go undetected until trading features break at
 * runtime, so a direct single-spy mount check provides an explicit,
 * granular guard.
 */
...
const tradingProviderSpy = vi.hoisted(() => vi.fn());
...
/**
 * TradingProvider: record the call via the spy and render children so
 * the rest of the tree can mount normally.
 */
vi.mock('@/context/TradingContext', () => ({
  TradingProvider: ({ children }: { children: React.ReactNode }) => {
    tradingProviderSpy();
    return <>{children}</>;
  },
}));
...
describe('RootLayout — TradingProvider mount guard', () => {
  it('mounts TradingProvider in the render tree', async () => {
    render(<RootLayout />);
    await waitFor(() => expect(tradingProviderSpy).toHaveBeenCalled());
  });

  it('mounts TradingProvider exactly once (no duplicates)', async () => {
    render(<RootLayout />);
    await waitFor(() => expect(tradingProviderSpy).toHaveBeenCalledTimes(1));
  });
});
